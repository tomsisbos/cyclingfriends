...
if (isset($_GET)) {
    ...
    if ($_GET['task'] == 'rides') {
        ...
        $activities = array_map(function ($activity) use ($db, $photos_number) {
            ...
            // Photos
            $getPhotos = $db->prepare("SELECT filename, datetime, elevation, featured, ST_X(point::geometry)::double precision as lng, ST_Y(point::geometry)::double precision as lat FROM activity_photos WHERE activity_id = ? ORDER BY featured::int DESC, RANDOM() LIMIT {$photos_number}");
