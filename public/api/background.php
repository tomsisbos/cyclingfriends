...
if (isAjax()) {
    ...
    if (isset($_GET['public-scenery-imgs'])) {
        ...
        $getPopularSceneryImages = $db->prepare("SELECT img.id, img.filename, img.date, sc.name, sc.city, sc.prefecture FROM scenery_photos AS img JOIN sceneries AS sc ON img.scenery_id = sc.id WHERE EXTRACT(MONTH FROM img.date) = EXTRACT(MONTH FROM NOW()) ORDER BY img.likes, RAND() DESC LIMIT {$imgs_number}");
