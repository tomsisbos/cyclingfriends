...
<div class="container smaller page">
	...
	<form id='form' class="container smaller inner" method="POST" action="<?php echo CFG_FORM_ACTION; ?>?id=<?= $_SESSION['edit-forms']['ride-id']; ?>&stage=<?= CFG_STAGE_ID + 1; ?>">
		<label class="form-label">Course setting method</label>
		<select id="formMethodSelect" class="form-select" name="method">
			<option <?php if (empty($_SESSION['edit-forms'][CFG_STAGE_ID]['method'])) {echo 'selected';} ?> hidden disabled value="none">Choose a course setting method...</option>
			<option <?php if ($_SESSION['edit-forms'][CFG_STAGE_ID]['method'] == 'pick') {echo 'selected';} ?> value="pick">By picking checkpoints on the map</option>
			<option <?php if ($_SESSION['edit-forms'][CFG_STAGE_ID]['method'] == 'import') {echo 'selected';} ?> value="import" disabled>By importing a *.gpx file</option>
			<option <?php if ($_SESSION['edit-forms'][CFG_STAGE_ID]['method'] == 'draw') {echo 'selected';} ?> value="draw">By selecting a route from my routes</option>
		</select>
				...
				<div class="mb-3 row g-2">
					...
					<div class="col-md">
						...
						<select class="form-select" id="terrain" name="terrain">
							<option <?php if($_SESSION['edit-forms'][CFG_STAGE_ID]['terrain'] == '1') {echo 'selected';} ?> value="1">Flat</option>
							<option <?php if($_SESSION['edit-forms'][CFG_STAGE_ID]['terrain'] == '2') {echo 'selected';} ?> value="2">Small hills</option>
							<option <?php if($_SESSION['edit-forms'][CFG_STAGE_ID]['terrain'] == '3') {echo 'selected';} ?> value="3">Hills</option>
							<option <?php if($_SESSION['edit-forms'][CFG_STAGE_ID]['terrain'] == '4') {echo 'selected';} ?> value="4">Mountains</option>
						</select>
					</div>
				</div>
				<div class="mb-3">
...
<script type="module" src="/rides/edit/course.js"></script>