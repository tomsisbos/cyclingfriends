...
if (isset($_POST['save'])) {
	...
	if (isset($_POST['privacy']) AND ($_POST['privacy'] != $ride->privacy)) {
		...
		if (is_array($ride->getParticipants())) foreach ($ride->getParticipants() as $participant_id) $ride->notify($participant_id, 'ride_privacy_change');
	}
	...
	if (isset($_POST['entry_start']) AND !empty($_POST['entry_start']) AND ($_POST['entry_start'] != $ride->entry_start)) {
		...
		if (is_array($ride->getParticipants())) foreach ($ride->getParticipants() as $participant_id) $ride->notify($participant_id, 'ride_entry_start_change');
	}
