...
class User extends Model {
    ...
    public function getPropicElement ($height = 60, $width = 60, $border_radius = 60) { ?>
        <div class="free-propic-container<?php if ($this->hasModeratorRights() || $this->isGuide()) echo ' official-badge'; ?>">
            <img style="height: <?= $height ?>px; width: <?= $width ?>px; border-radius: <?= $border_radius ?>px;" class="free-propic-img" src="<?= $this->getPropicUrl() ?>" />
        </div> <?php
    }
    ...
    public function sendMessage ($receiver, $message){        
        ...
        $addMessage->execute(array($this->id, $receiver->id, $message, (new DateTime('now'))->setTimezone(new DateTimeZone('Asia/Tokyo'))->format('Y-m-d H:i:s')));
    }
