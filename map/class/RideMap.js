...
export default class RideMap extends GlobalMap {
    ...
    apiUrl = '/actions/rides/new/courseApi.php'
    cursor = 0
    data = {
        checkpoints: []
    }
    ...
    async updateSession (variable) {
        ...
        return new Promise ((resolve, reject) => {
            ...
            if (variable.data.checkpoints) {
                ...
                variable.data.checkpoints.forEach(checkpoint => {
                    var cleanCheckpoint = {}
                    for (var key in checkpoint) {
                        if (key != 'marker')
                        cleanCheckpoint[key] = checkpoint[key]
                    }
                    cleanVariable.data.checkpoints.push(cleanCheckpoint)
                } )
