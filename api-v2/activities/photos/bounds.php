...
ROW_NUMBER() OVER (ORDER BY ABS(EXTRACT(MONTH FROM ap.datetime) - {$month}), ap.datetime DESC, RANDOM() ASC) as number
...
ORDER BY ABS(EXTRACT(MONTH FROM ap.datetime) - {$month}), ap.datetime DESC, RANDOM() ASC
LIMIT ?");
$getActivityPhotosFromBoundingBox->execute([$_GET['bounds'][0], $_GET['bounds'][1], $_GET['bounds'][2], $_GET['bounds'][3], $_GET['limit']]);
