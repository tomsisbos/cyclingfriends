...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $activities = array_map(function ($activity) use ($db, $photos_number) {
        ...
        // Photos
        $getPhotos = $db->prepare("SELECT filename, EXTRACT(EPOCH FROM datetime) as datetime, featured, ST_X(point::geometry)::double precision as lng, ST_Y(point::geometry)::double precision as lat, privacy FROM activity_photos WHERE activity_id = ? ORDER BY featured::int DESC, RANDOM() LIMIT {$photos_number}");
