<div class="rdr-container-buttons"> <?php
	...
	if ($connected_user->id !== $user->id) {
		...
		if (!$connected_user->follows($user)) { ?>
			<button data-action="follow" data-id="<?= $user->id ?>" class="rdr-button success js-follower">
		...
		// If connected user already follows the rider
			...
			<button data-action="unfollow" data-id="<?= $user->id ?>" class="rdr-button danger js-follower">
	...
	if ($user->isFriend($connected_user)) { ?>
		<button data-action="remove" data-id="<?= $user->id ?>" data-login="<?= $user->login; ?>" class="rdr-button danger js-friend">
	...
	// If the rider has sent a request to connected user
		...
		<button data-action="accept" data-id="<?= $user->id ?>" data-login="<?= $user->login; ?>" class="rdr-button success js-friend">
			<span class="iconify-inline" data-icon="eva:person-done-outline" style="color: white;" data-width="20" data-height="20"></span>
			Accept
		</button>
		<button data-action="dismiss" data-id="<?= $user->id ?>" data-login="<?= $user->login; ?>" class="rdr-button danger js-friend">
			<span class="iconify-inline" data-icon="eva:person-remove-outline" style="color: white;" data-width="20" data-height="20"></span>
			Dismiss
		</button> <?php
	// If connected user has sent a request to the rider
	} else if (in_array($connected_user->id, $user->getRequesters())){ ?>
		<button data-id="<?= $user->id ?>" data-login="<?= $user->login; ?>" class="rdr-button no-click">
			Request sent...
		</button> <?php
	// If the rider is not friend with connected user (and is not connected user himself)
	} else if ($_SESSION['id'] != $user->id){ ?>
		<button data-action="add" data-id="<?= $user->id ?>" data-login="<?= $user->login; ?>" class="rdr-button success js-friend">
