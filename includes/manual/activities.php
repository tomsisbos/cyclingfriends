...
$content = [
    [
        ...
        'content' => [
            [
                'title' => '走行データのインポート',
                'text' => [
                    'ベータフェーズの段階では、走行データの読み取りをファイルアップロード形式でしか対応しておりません。そのため、走行データを記録元のアプリ／サイクルコンピューターからダウンロードするか、第三者のデータ管理サービス（Strava等）でエクスポートする必要があります。',
                    '今後は、自社開発のモバイルアプリから記録する方法と、サイクルコンピューターと同期し自動インポートする方法の導入を検討していく予定です。',
                ...
                'content' => [
                    ...
                    [
                        ...
                        'text' => [
                            ...
                            'point' => 'アクティビティ自体を「非公開」に設定した場合であっても、写真単体を「公開」に設定することができます。アクティビティ自体を非公開にしたいけど、絶景スポットを作成したり、写真を絶景スポットに追加したい場合には、写真が公開に設定されている必要があります。',
    ...
    [
        ...
        'text' => [
            '新規のアクティビティを作成し保存すると、アクティビティ紹介ページが自動作成されます。アクティビティ設定が「公開」となっている場合、このページは誰にでも閲覧できます（サインインされていないユーザーを含む）。このページをFACEBOOK、TWITTER、INSTAGRAMに共有できます。但し、作成者がプレミアムユーザーである場合の除き、サインインされていないユーザーに表示される地図は静的地図になります。'
        ],
        'content' => [
            [
                'title' => 'ヘッダー',
